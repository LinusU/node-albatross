    ...
    switch (this.op) {
      ...
      case 'update':
        if (this.opts.multi === true) {
          ...
          return col.updateMany(this.args[0], this.args[1], this.opts).then(cbWriteCount, logError)
        } else {
          debug('updateOne(%o, %o)', this.args[0], this.args[1])
          return col.updateOne(this.args[0], this.args[1], this.opts).then(cbWriteCount, logError)
        }

      case 'remove':
        if (this.opts.single === true) {
          debug('removeOne(%o)', this.args[0])
          return col.removeOne(this.args[0], this.opts).then(cbWriteCount, logError)
        } else {
          debug('removeMany(%o)', this.args[0])
          return col.removeMany(this.args[0], this.opts).then(cbWriteCount, logError)
        }
